...
```{r setuprmd, include=FALSE, results='hide'}
...
```{r side1, echo=FALSE, fig.cap="Fig1.Bike Lane Types in New York City", out.width = '100%'}
knitr::include_graphics("E:/NYCDOT/vis draft/images/lanetype_table.png")
```
...
```{r side2, echo=FALSE, fig.cap="Fig2.Bike Lanes by Type", out.width = '100%'}
knitr::include_graphics("E:/NYCDOT/vis draft/images/plot_lanes_bytype.png")
```

```{r side3, echo=FALSE, fig.cap="Fig3.Bike Lanes by Borough", out.width = '100%'}
knitr::include_graphics("E:/NYCDOT/vis draft/images/plot_lanes_byBoro.png")
```
...
```{r bikepricess, message=FALSE, warning=FALSE, results='hide'}

#import bike lane data
bikelane_17d<- st_read("E:/NYCDOT/Feb23/data/bikelane_17d/bikelane_17d.shp")
bikelane_20d<- st_read("E:/NYCDOT/Feb23/data/bikelane_17d/bikelane_20d.shp")


new_18 <- st_read("E:/NYCDOT/Feb23/data/bikelane_1718Diff_2/bikelane_1718Diff_2.shp")
new_19 <- st_read("E:/NYCDOT/Feb23/data/bikelane_1819Diff_2/bikelane_1819Diff_2.shp")
new_20 <- st_read("E:/NYCDOT/Feb23/data/bikelane_1920Diff_2/bikelane_1920Diff_2.shp")


boroughs<- st_read("E:/NYCDOT/Feb23/data/borough.geojson")
ntas<- st_read("E:/NYCDOT/Feb23/data/ntas.geojson")

parks<- st_read("E:/NYCDOT/Feb23/data/parks_nyc/parks_nyc.shp")
...
ggplot()+
  ...
  labs(title='Fig4. New York City Bike Lanes by Type 2021')+
  facet_wrap(~LaneType)
```
...
```{r side4, echo=FALSE, fig.cap="Fig5.Bike Lanes by Borough and Type", out.width = '100%'}
knitr::include_graphics("E:/NYCDOT/vis draft/images/plot_lanes_byBoroLaneType.png")
```
...
ggplot(data=combo_2, aes(x=Year, y=pct, label=scales::percent(pct)))+
  ...
  labs(title='Fig6.Bike Lanes Added During the Study Period', x='Year Built', y = 'Percent of Network')+plotTheme
...
ggplot()+
  ...
  scale_colour_manual(values = c('#F55536','#f57f36', '#f7ac2a', '#fcd93a'))+
  facet_wrap(~Year)+
  labs(title='Fig7.New Bike Lanes During Study Period', subtitle='By Year and Type')+
  mapTheme()
```
\
...
ggplot()+
  ...
  scale_colour_manual(values = c('#F55536','#f57f36', '#f7ac2a', '#fcd93a'))+
  labs(title='Fig8.2018: New Bike Lanes by Type')+ mapTheme()
...
ggplot()+
  ...
  scale_colour_manual(values = c('#F55536','#f57f36', '#f7ac2a', '#fcd93a'))+
  labs(title='Fig9.2019: New Bike Lanes by Type')+ mapTheme()
...
ggplot()+
  ...
  scale_colour_manual(values = c('#F55536','#f57f36', '#f7ac2a', '#fcd93a'))+
  labs(title='Fig10.2020: New Bike Lanes by Type') + mapTheme()
```
\
...
```{r side5, echo=FALSE, fig.cap="Fig11.NYC Citibike Network Extent 2021", out.width = '100%'}
knitr::include_graphics("E:/NYCDOT/vis draft/images/citibike_extent.png")
```
\
...
new_18_table<- new_18_clean%>%
  ...
  arrange(desc(Length))%>%
  head(15)%>%
  kbl(caption='Table1.Longest New Bike Lanes in 2018')%>%
  kable_styling()
new_18_table
...
new_19_table<- new_19_clean%>%
  ...
  arrange(desc(Length))%>%
  head(20)%>%
  kbl(caption='Table2.Longest New Bike Lanes in 2019')%>%
  kable_styling()
new_19_table
...
new_20_table<- new_20_clean%>%
  ...
  arrange(desc(Length))%>%
  head(20)%>%
  kbl(caption='Table3.Longest New Bike Lanes in 2020')%>%
  kable_styling()
new_20_table
...
```{r wrongoutside, echo=FALSE, fig.cap="Fig12.Wrong trajectories falling outisde of NYC-1", out.width = '80%'}
...
```{r wrongoutside2, echo=FALSE, fig.cap="Fig13.Wrong trajectories falling outisde of NYC-2", out.width = '80%'}
...
```{r bufferchoice, echo=FALSE, fig.cap="Fig14.Buffer Choice", out.width = '90%'}
...
```{r pointIntersect, echo=FALSE, fig.cap="Fig15.Point on Several Roads", out.width = '90%'}
...
ggplot(dayTrend2018, aes(x=day, y=n, color=as.character(month))) + 
  geom_line() +
  labs(title = "Fig16.Daily Trend in year 2018",
       x = "Day",
       y = "Count",
       colour="Month") +
  plotTheme +
...
ggplot(hourTrend2018, aes(x=rhour, y=n, color=as.character(month))) + 
  geom_line() +
  labs(title = "Fig17.Hourly Trend in year 2018",
       x = "Hour",
       y = "Count",
       colour="Month") +
  plotTheme +
...
ggplot(wdayTrend2018, aes(x=wday, y=n, color=as.character(month))) + 
  ...
  labs(title = "Fig18.Weekday Trend in year 2018",
       x = "Day of Week",
       y = "Count",
       colour="Month") +
  plotTheme +
...
ggplot() +
  ...
  scale_colour_viridis(direction = -1,
                       ...
                       na.value="#D4D4D4") +
  labs(title = "Fig19.Bike Trip Counts in July 22pm") +
  mapTheme()

```

\
From the figure, we could see that 2019 have much more data than 2018 overall. Also, bike lanes where have more trips in 2018 also have more trips in 2019.
\
...
bike18new <- st_read("E:/NYCDOT/Feb23/data/new18_clean_buffered/new_18_clean_15ft.shp") %>%
  st_transform(st_crs(July2018))

#assign Street Name to new bike lanes
bike18new_M <- bike18new %>%
  filter(Borough == "Manhattan")

#change column name
colnames(bike18new_M)[1] <- "Street"
...
The following table shows the number of added trips on added bike lanes. 
```{r showtable}
new.num %>% 
  select(Street, Diff) %>% 
  kbl(caption='Table4.Added Trips on added bike lanes') %>%
  kable_styling()

```
A bar graph is also made to visualize how many more trips on the added bike lanes.
```{r showdiffvis}
ggplot(data=new.num, aes(x=Street, y=Diff)) +
  geom_bar(stat="identity", fill="#99CCFF", color="#66B2FF") +
  theme(
    # Remove grid lines
  ...
  labs(title = "Fig20.Trip Differences of Added Bike Lanes",
       subtitle = "Between year 2018 and year 2019 during July 22pm",
       y = "Trip Differences") + plotTheme
...
#bike data
chel_07_2018 <- st_read("E:/NYCDOT/Feb23/data/Chelsea_EastHarlem/chelsea_1807.shp") %>% st_transform(crs=4326)
chel_07_2019 <- st_read("E:/NYCDOT/Feb23/data/Chelsea_EastHarlem/chelsea_1907.shp") %>% st_transform(crs=4326)
Har_07_2018 <- st_read("E:/NYCDOT/Feb23/data/Chelsea_EastHarlem/harlem_1807.shp") %>% st_transform(crs=4326)
Har_07_2019 <- st_read("E:/NYCDOT/Feb23/data/Chelsea_EastHarlem/harlem_1907.shp") %>% st_transform(crs=4326)
#neighborhood
chelsea <- st_read("E:/NYCDOT/Feb23/data/Chelsea_for7thAve/Chelsea_for7thAve.shp") %>% st_transform(crs=4326)
eastHarlem <- st_read("E:/NYCDOT/Feb23/data/East_Harlem_CD/East_Harlem_CD.shp")%>% st_transform(crs=4326)
#15 ft road buffer within the neighborhood boundary
road15_chel <- st_read("E:/NYCDOT/Feb23/data/roadBuffer/neighborhoodBuffer/road15_chel.shp") %>% st_transform(crs=4326)
road15_har <- st_read("E:/NYCDOT/Feb23/data/roadBuffer/neighborhoodBuffer/road15_har.shp") %>% st_transform(crs=4326)

```
```{r newbikenighbor, message=FALSE, warning=FALSE, results='hide'}
#added lanes in neighborhood
newLane_chel <- bike18new_M[chelsea,] %>% select(Street,geometry) %>% mutate(isNew = 1)
newLane_har <- bike18new_M[eastHarlem,]%>% select(Street,geometry) %>% group_by(Street) %>% tally() %>% mutate(isNew = 1) %>% select(-n)
```
...
```{r changelang2, message=FALSE, warning=FALSE, include=FALSE, results='hide'}
Sys.setlocale("LC_TIME", "English")

```

```{r vis 1}
grid.arrange(top="Fig21.Ridership Trend by Hour and by Day",
  ggplot(chel_07_2018 %>% 
           st_drop_geometry() %>%
...
ggplot(rbind(
  ...
  labs(title="Fig22.Ridership pr hour in a day", y="Count", x="Hour", color="Labels")
```
...
ggplot(rbind(
  ...
  labs(title="Fig23.Ridership by Weekday", y="Count", x="Day of Week", color="Labels")
```

Next, we study the ridership changes in two neighborhoods seperately.
...
har_Diff1819 <- road15_har %>% 
  ...
  select(-trip18,-trip19) %>% 
  merge(.,newLane_har %>%
          st_drop_geometry(),by.x="Street",by.y="Street",all.x=T) %>%
  replace(.,is.na(.),0) 
...
ggplot() + 
  ...
  scale_color_viridis(direction = 1,
                      ...
                      na.value = "#D4D4D4")+
  labs(title = "Fig24.Trip Count per road, Chelsea, July") + facet_wrap(~Year)+ mapTheme()
```
...
grid.arrange(
  ...
  scale_color_viridis(direction = 1,
                      discrete = F, option = "viridis",
                      na.value = "#D4D4D4")+
  labs(title = "Fig25.Ridership increase,
       Chelsea, 2018-2019") +
  mapTheme(),
  ggplot()+
  geom_sf(data = chelsea,fill = "#f7f5f5",col = NA)+
  ...
  scale_color_viridis(direction = 1,
                      discrete = F, option = "viridis",
                      na.value = "#D4D4D4")+
  labs(title = "Fig26.Ridership increase on new 
...
ggplot() + 
  ...
  scale_color_viridis(direction = 1,
                      discrete = F, option = "viridis",
                      na.value = "#D4D4D4")+
  labs(title = "Fig27.Trip Count per road in July, East Harlem") +
  facet_wrap(~Year)+
  mapTheme()
```
...
grid.arrange(
  ...
  scale_color_viridis(direction = 1,
                      discrete = F, option = "viridis",
                      na.value = "#D4D4D4")+
     labs(title = "Fig28.Ridership increase, East Harlem, 
       2018-2019") +
  mapTheme(),
  ggplot()+
  geom_sf(data = eastHarlem,fill = "#f7f5f5",col = NA)+
  ...
  scale_color_viridis(direction = 1,
                      discrete = F, option = "viridis",
                      na.value = "#D4D4D4")+
  labs(title = "Fig29.Ridership increase on new 
