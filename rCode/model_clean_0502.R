...
bike20d_buffer <- bike20d_buffer %>%
  select(Street,SegmentID, FeatureTyp, SegmentTyp, NonPed,
...
###Caution: This step was really time consuming, costing around 40 mins
###So I saved the result.
...
info.Aug_nh <- st_join(info.Aug, neighborhood %>% st_transform(st_crs(info.Aug)), join=st_intersects, left=TRUE,largest = TRUE)
...
ggplot() +
  ...
  geom_sf(data = info.Aug %>% filter(Count < 50), aes(colour =Count)) +
...
info.Aug_osm<- info.Aug_osm %>%
  mutate(office_nn5 = nn_function(st_coordinates(st_centroid(info.Aug_osm)), st_coordinates(office), 5))
...
reg_compare<-lm(Count ~ bikeLaneLv + dist.lane + Number_Tra + StreetWidt + TRUCK_ROUT + citibike.Buffer_small + isMH +
                  pWhite + MedRent + pNoVeh + pSubway + 
