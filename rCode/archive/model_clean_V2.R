...
Aug_pnts <- st_read("BikeTrips/pointData/Aug20_pnts/tripPoints_Aug20.shp") 
...
bike20d_buffer <- st_read("lion2020d_buffer15.shp") %>%
  st_transform(st_crs(Aug_pnts))
bike20d_buffer <- st_read("Infrastructure/Lion/lion20d_buffer/lion20d_buffer.shp") %>%
  st_transform(st_crs(Aug_pnts))
...
points_clean <- Aug_pnts[bike20d_buffer,] %>% 
  ...
  st_join(.,bike20d_buffer,join = st_intersects, left = T,largest = TRUE)

st_write(points_clean,"Aug_pnts_2wk.shp")
# points_clean <- st_read("Aug_pnts_2wk.shp")
...
rm(Aug_pnts,bike20d_buffer_drop,borough,tripCount)
...
bike.nh <- st_join(info.Aug, neighborhood %>% st_transform(st_crs(info.Aug)), join=st_intersects, left=TRUE,largest = TRUE)
info.Aug <- bike.nh %>% select(-(ntacode:county_fips),-(shape_leng:boro_code))
####
####
####
# check the join of hte neighborhood and the nhCount here before proceed to the next step
bikeCount.nh <- bike.nh %>%
...
## I edit this -- change median to 0
...
# retail
##??? this step seems to join centriod of each road segemnt with the retail points
## Should we join poi to the census tract and count tract level summary?
...
# office
##?? The same question for the office?
...
where <- info.Aug$geometry[is.na(info.Aug$ntaname)]
mapview(list(where,neighborhood))
...
correlation.long_e <-
  select(st_drop_geometry(info.Aug), c(sidewalk_caf_nn1, sidewalk_caf_nn2, sidewalk_caf_nn3,#sidewalk_caf.Buffer,
                                           jobs_in_tract, 
